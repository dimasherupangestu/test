...
<script>
	function save_sort(id) {
		...
		$.ajax({
			...
			data: {
				sort: sort
			}, // Perubahan: Mengirim data POST dengan format object
			success: function(result) {
				var response = JSON.parse(result); // Perubahan: Parsing respons JSON
				if (response.status === 'success') {
					Swal.fire('Berhasil', response.message, 'success'); // Perubahan: Menampilkan pesan sesuai respons
				} else {
					Swal.fire('Gagal', response.message, 'error'); // Perubahan: Menampilkan error sesuai respons
				}
			},
			error: function() {
				Swal.fire('Error', 'Terjadi kesalahan pada server', 'error'); // Perubahan: Tambah error handling
			}
		});
	}
