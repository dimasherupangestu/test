...
abstract class BaseController extends Controller
{
    ...
    protected $session;
    protected $tokenName = 'order_token';
    ...
    public function initController(RequestInterface $request, ResponseInterface $response, LoggerInterface $logger)
    {
        ...
        if (
            preg_match("/webzip|httrack|wget|FlickBot|downloader|production
    ...
    protected function isFormProcessed($formIdentifier)
    {
        return $this->session->get('formProcessed.' . $formIdentifier);
    }

    protected function setFormProcessed($formIdentifier)
    {
        $this->session->set('formProcessed.' . $formIdentifier, true);

        // Atur waktu kadaluwarsa sesuai dengan kebutuhan
        $expirationTime = time() + 3; // Contoh: Formulir dapat diproses kembali setelah 5 detik
        $this->session->set('formProcessedExpiration.' . $formIdentifier, $expirationTime);
    }

    protected function clearExpiredFormProcessedStatus($formIdentifier)
    {
        $expirationTime = $this->session->get('formProcessedExpiration.' . $formIdentifier);

        if ($expirationTime && $expirationTime < time()) {
            $this->session->remove('formProcessed.' . $formIdentifier);
            $this->session->remove('formProcessedExpiration.' . $formIdentifier);
        }
    }
    
    protected function generateOrderToken()
    {
    ...
    protected function validateOrderToken($token)
    {
        $sessionToken = $this->session->get($this->tokenName);
        if ($token !== $sessionToken) {
            return false;
        }
        // Invalidate the token after validation to prevent reuse
