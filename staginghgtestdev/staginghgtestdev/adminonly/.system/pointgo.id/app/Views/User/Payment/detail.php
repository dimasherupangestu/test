                            ...
                            <div class="row">
                                <div class="col-sm-6">
									...
									if ($orders['payment_gateway'] == 'Tripay') {
										...
										} else if (in_array($orders['method_code'], array('QRIS', 'QRIS2'))) {
											$fee = 750;
											$harga = ($orders['price'] * 1.007) + $fee;
										} else if (in_array($orders['method_code'], array('OVO', 'SHOPEEPAY'))) {
											$fee = 0;
											$harga = ($orders['price'] * 1.03) + $fee;
										} else if (in_array($orders['method_code'], array('BSIVA', 'BNIVA', 'PERMATAVA', 'MANDIRIVA', 'BRIVA'))) {
											$fee = 4250;
										...
										if (in_array($orders['method_code'], array('OV', 'DA', 'LA'))) {
									...
									<?php if (($orders['payment_type'] == 'QRIS' && $orders['payment_gateway'] == 'Xendit')  || ($orders['payment_type'] == 'QRIS' && $orders['payment_gateway'] == 'Ipaymu')): ?>

									<div class="pb-4"><b> SCAN QR CODE dibawah ini </b> <br>
										<img src="https://api.qrserver.com/v1/create-qr-code/?data=<?= $orders['payment_code']; ?>&amp;size=250x250" class="mt-3 mb-1 p-4 bg-white" style="border-radius:15px;" alt="" title="" />
									</div>

									<?php elseif ((in_array($orders['payment_type'], ['QRIS',]) && $orders['payment_gateway'] == 'Tripay') || (in_array($orders['payment_type'], ['QRIS']) && $orders['payment_gateway'] == 'Linkqu')): ?>
									...
									<?php elseif ($orders['payment_type'] == 'Convenience Store'): ?>
                                ...
                                <div class="col-sm-6">
                                    ...
                                    <div class="pb-4"> Rincian Pesanan <h5><?= $orders['games']; ?> - <?= $orders['product']; ?></h5>
                                        ...
                                        <?php elseif ($orders['zone_id'] == 'jokigendong'): ?>
                                        <table>
                                            <?php foreach (json_decode($orders['user_id'], true) as $key => $value): ?>
                                            <tr>
                                                <th><?= str_replace([
