...
class Sistem extends BaseController
{
    ...
    public function status_update_ranking_today() {
        
    $todayStart = date('Y-m-d') . ' 00:00:00';

    $orders = $this->M_Base->data_where_array('orders', [
        'date_create >=' => $todayStart,
        'status' => 'Success',
    ]);
    ...
    foreach ($orders as $index => $order) {
        if (!empty($order['username'])) {
            ...
            if (!isset($groupedData[$username])) {
                $groupedData[$username] = array(
                    'no' => count($groupedData) + 1,
                    'price' => $order['price'],
                    'username' => $username,
                );
            } else {
                $groupedData[$username]['price'] += $order['price'];
            }
        }
    }
    ...
    $excluded_ids = array('16', '17', '20', '10');

    $newData = array(); 
    $validRowNumber = 0; 
    
    foreach ($data as $index => $order) {
        $user_info = $this->M_Base->data_where('users', 'username', $order['username']);
        $full_name = !empty($user_info) ? $user_info[0]['fullname'] : '';
        $user_id = !empty($user_info) ? $user_info[0]['id'] : '';
    
        if (!empty($full_name) && !in_array($user_id, $excluded_ids)) {
            $order['no'] = ++$validRowNumber; 
    ...
    $today_ranking = $this->M_Base->all_data('today_ranking');
    
    $this->M_Base->data_truncate('today_ranking');
    
    foreach ($insertData as $insertRow) {
        $this->M_Base->data_insert('today_ranking', $insertRow);
    }
    ...
    public function status_update_ranking_weekly() {
    ...
    $allOrders = [];
    $weeks = [
        ['start' => date('Y-m-01 00:00:00'), 'end' => date('Y-m-07 23:59:59')],
        ['start' => date('Y-m-08 00:00:00'), 'end' => date('Y-m-15 23:59:59')],
        ['start' => date('Y-m-16 00:00:00'), 'end' => date('Y-m-23 23:59:59')],
        ['start' => date('Y-m-24 00:00:00'), 'end' => $endOfMonth],
    ];

    foreach ($weeks as $week) {
        $orders = $this->M_Base->data_where_array('orders', [
            'date_create >=' => $week['start'],
            'date_create <=' => $week['end'],
            'status' => 'Success',
        ]);
    ...
    $data = $newData;
    $this->M_Base->data_truncate('weekly_ranking');
    foreach ($insertData as $insertRow) {
        $this->M_Base->data_insert('weekly_ranking', $insertRow);
    }
    ...
    public function status_update_ranking_monthly() {
    $startOfMonth = date('Y-m-01 00:00:00');
    $today = date('Y-m-d H:i:s');
    
    $orders = $this->M_Base->data_where_array('orders', [
        'date_create >=' => $startOfMonth,
        'date_create <=' => $today,
        'status' => 'Success',
    ]);
    ...
    $monthly_ranking = $this->M_Base->all_data('monthly_ranking');
    
    $this->M_Base->data_truncate('monthly_ranking');
    
    foreach ($insertData as $insertRow) {
        $this->M_Base->data_insert('monthly_ranking', $insertRow);
    }
    ...
    public function statuslinkqu()
{
                ...
                if ($order['method'] !== 'API Saldo Akun Seller Digiflazz' ){
                        ...
                        if (isset($result)) {
                            if ($result['status'] == 0) {
								...
								// LIMIT STATUSAPIGAMES10
								
							 //$product_id = $order['product_id'];
        //                         $product_data = $this->M_Base->data_where('product', 'id', $product_id);
        //                         if ($product_data[0]['limitflashsale'] > 0) {

        //                         $stok = $product_data[0]['limitflashsale'] - 1;
    ...
    public function statusprov(){
       $ag =  $this->M_Base->post('https://v1.apigames.id/v2/transaksi/status', []);
       echo json_encode($ag).'<br>';
       
       $lg =  $this->M_Base->post('https://www.lapakgaming.com/api/product?category_code=FF', []);
       echo  json_encode($lg);
    }
...
// 	}
    ...
    public function callbackproductlg()
	{
        ...
        $allowed_ips = array('188.166.205.40');
		...
		if (in_array($ip_address, $allowed_ips)) {
    		if ($result) {
    			$product = $this->M_Base->data_where_array('product', [
    				'sku' => $result['data']['code'],
    				'provider' => 'LG'
    			]);
			
				$gamesx = $this->M_Base->data_where_array('games', [
					'id' => $product[0]['games_id'],
					'provider' => 'LG',
				]);
				
				$product_data = [
					'status' => $result['data']['status'] == 'available' ? 'On' : 'Off',
					'check_code' => date('Y-m-d G:i:s').' LG',
				];
...
$result = $this->M_Base->ag_v2_order($product, $userid, $zoneid, $orderid);
    ...
    public function gamesvocadata()
	{
        ...
        $table = '<table>';
        $table .= '<thead style="text-align:left"><tr><th>Kode Games</th><th hidden>Kode Games</th><th>Nama Games</th><th>Tipe Games</th></tr></thead>';
        $table .= '<tbody>';
        foreach ($result['data'] as $game) {
            ...
            $table .= '<tr><td>' . $id . '</td><td hidden>' . $code . '</td><td>' . $name . '</td><td>' . $variant . '</td></tr>';
        }
        $table .= '</tbody>';
        $table .= '</table>';
        ...
        if (is_array($data)) {
            ...
            if (count($orders) === 1) {
                ...
                if ($data['status'] == 'Success') {
                    ...
                    if ($orders[0]['method'] == 'API Saldo Akun Seller Digiflazz') {
                        $this->M_Base->sendPayloadDf($orders[0]['api_trx_id'], 'Success');
                    }
                } else if ($data['status'] == 'Processing') {
                    $status = 'Processing';
                    $ket = $data['sn'];
