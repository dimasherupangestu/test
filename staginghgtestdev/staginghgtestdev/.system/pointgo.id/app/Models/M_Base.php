...
class M_Base extends Model
{
	...
	public function data_update_atomic_custom($table, $id, $incrementValue, $column = 'balance')
	{
		...
		try {
			...
			$query = $this->db->query("SELECT $column FROM $table WHERE id = ? FOR UPDATE", [$id]);
			$result = $query->getRow();

			if ($result) {
				if (is_numeric($incrementValue)) {
					// Jika incrementValue adalah angka, periksa apakah cukup untuk pengurangan
					if ($result->$column >= -$incrementValue) {
						// Melakukan operasi atomik untuk mengupdate nilai
						$this->db->query("UPDATE $table SET $column = $column + ? WHERE id = ?", [$incrementValue, $id]);
					} else {
						// Rollback transaksi jika nilai tidak mencukupi
						$this->db->transRollback();
						return false;
					}
				} else {
					// Jika incrementValue bukan angka, anggap sebagai nilai string untuk pembaruan langsung
					$this->db->query("UPDATE $table SET $column = ? WHERE id = ?", [$incrementValue, $id]);
				}
	...
	// Fungsi untuk mengupdate status produk
	public function updateProductStatus($productId, $status)
	{
		// Panggil method dari model untuk melakukan update status produk
	...
	public function wa($phone, $message)
	{
		...
		$log_file_name = 'log_wa_otpmember' . date("j.n.Y") . '.txt'; // Include time in the log file name
		$file = fopen(WRITEPATH . 'logs/' . $log_file_name, "a");
		fwrite($file, date('Y-m-d H:i:s') . ' ' . ($response) . "\n"); // Include time in the log message
		fclose($file);
	}
...
Terima kasih.
	...
	public function all_data_asc_games($table, $limit = null)
	{
		if ($limit) {
			// Mengurutkan data berdasarkan kolom 'games' secara ascending dengan limit
			return $this->db->table($table)->orderBy('games', 'ASC')->limit($limit)->get()->getResultArray();
		} else {
			// Mengurutkan data berdasarkan kolom 'games' secara ascending tanpa limit
			return $this->db->table($table)->orderBy('games', 'ASC')->get()->getResultArray();
		}
	}

	public function get_paginated_api_data_voca($games = null, $search, $offset, $limit = null)
	{
		$merchantId = $this->u_get('voca_merchantid');
		$secretKey = $this->u_get('voca_secretkey');

		$endpoint = "/products/" . $games . "/items";
		$signature = hash_hmac('sha256', $merchantId . $endpoint, $secretKey);

		$publik = $this->u_get('margin_voca_publik');
		$silver = $this->u_get('margin_voca_silver');
		$gold = $this->u_get('margin_voca_gold');
		...
		$curl = curl_init();
		$curl_options[CURLOPT_URL] = 'https://api-bisnis.vocagame.com/v1/core/products/' . $games . '/items?signature=' . $signature . '';
		...
		if (curl_errno($curl)) {
			...
			curl_close($curl);
			return array('total' => 0, 'rows' => array());  // kembalikan array kosong sebagai gantinya
		}
		curl_close($curl);

		$log_file_name = 'log_api_response_vg' . date("j.n.Y") . '.txt';
		$file = fopen(WRITEPATH . 'logs/' . $log_file_name, "a");
		fwrite($file, date('Y-m-d H:i:s') . ' API response: ' . $response . "\n");
		fclose($file);

		// Decode respons JSON dan periksa kesalahan
		...
		// Verifikasi kunci data dan products
		if (!isset($data['data'])) {
			log_message('error', 'Unexpected data structure: ' . print_r($data, true));
			return array('total' => 0, 'rows' => array());  // kembalikan array kosong sebagai gantinya
		}
		...
		// Loop melalui semua produk untuk memfilter produk yang tersedia
		...
		// Jika ada parameter pencarian, lakukan pencarian di array produk yang tersedia
		if ($search) {
			$availableProducts = array_filter($availableProducts, function ($product) use ($search) {
				return strpos(strtolower($product['name']), strtolower($search)) !== false;
			});
		}

		// Update total produk yang tersedia setelah pencarian
		$totalAvailableProducts = count($availableProducts);

		// Terapkan offset dan limit
		$availableProducts = array_slice($availableProducts, $offset, $limit);
	...
	public function insert_massal_Product_voca($gamesvoca, $gamesdb)
	{

		$merchantId = $this->u_get('voca_merchantid');
		$secretKey = $this->u_get('voca_secretkey');

		$endpoint = "/products/" . $gamesvoca . "/items";
		$signature = hash_hmac('sha256', $merchantId . $endpoint, $secretKey);
		...
		$silver = $this->u_get('margin_voca_silver');
		$gold = $this->u_get('margin_voca_gold');
		$platinum = $this->u_get('margin_voca_platinum');
		$diamond = $this->u_get('margin_voca_diamond');
		...
		// Inisialisasi dan eksekusi cURL
		$curl = curl_init();
		$curl_options[CURLOPT_URL] = 'https://api-bisnis.vocagame.com/v1/core/products/' . $gamesvoca . '/items?signature=' . $signature . '';
		curl_setopt_array($curl, $curl_options);
		$response = curl_exec($curl);
		...
		$log_file_name = 'log_api_response_vg_massal' . date("j.n.Y") . '.txt';
		$file = fopen(WRITEPATH . 'logs/' . $log_file_name, "a");
		fwrite($file, date('Y-m-d H:i:s') . ' API response: ' . $response . "\n");
		fclose($file);
		...
		if (count($result['data']) > 2) {
			usort($result['data'], [$this, 'compareProducts']);
			$counter = 1;

			foreach ($result['data'] as $loop) {

				// $product = $this->data_where_array('product', [
				// 	'sku' => $loop['id'],
				// 	'provider' => 'VG'
				// ]);

				$gamesx = $this->data_where('games', 'games', $gamesdb);
				...
				if ($loop['isActive'] == true) {
					if (count($gamesx) === 1) {
						...
						$this->data_insert('product', [
							'games_id' => $gamesx[0]['id'],
							'product' => $loop['name'],
							'product_id' => $gamesvoca,
							...
							'price_diamond' => ($loop['price'] / 100 * ($diamond + 100)),
							'provider' => 'VG',
							'sku' => $loop['id'],
							'status' => 'On',
							'sort' => $counter,
						...
						$counter++;
						echo $loop['name'] . ' VG data_insert ON <br>';
	...
	public function get_categories_data_voca()
	{
		$merchantId = $this->u_get('voca_merchantid');
		$secretKey = $this->u_get('voca_secretkey');
		$endpoint = "/products";
	...
	public function sort_product_api($a, $b)
	{
